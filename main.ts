...
console.log(chalk.yellow("                                               ABC Tuition Center                     "));
console.log(chalk.yellow("                                          --------------------------------                   "));
console.log(chalk.green("                       At ABC, we provide tailored, high-quality education with small class sizes."));
console.log(chalk.green("                          Our dedicated instructors foster a supportive learning environment."));
...
console.log(chalk.green(`Welcome to ABC tution center ${chalk.red.underline(studentInfo.name.toUpperCase())}`))
...
Schedule.forEach((entry: Schedule) => {
    console.log(
      `| ${entry.day.padEnd(13)}| ${entry.time.padEnd(15)}| ${entry.subject.padEnd(10)}| ${entry.teacherName.padEnd(12)}| ${entry.fees.padEnd(5)}|`
    );
  });
...
}else{console.log(chalk.bgCyan(`You have  successfully enrolled in:`));}

enrolledSubjects.forEach((subject:string) => {
    switch (subject) {
        case "English" :
...
// make student Id
const studentID =  `${studentInfo.name.replace(/\s+/g).substring(0, 3).toUpperCase()}${randomNum}${subjects.subjecT.map((subject:string) => subject[0]).join('')}`;
...
console.log(chalk.cyan(`Center: ABC Tution center\nStudentID: ${studentID}\nYour Balance: ${currentbalance.balance}`))
